...
def init_custom_fonts(config: dict):
    ...
    # Go up from nrhof/ui/fonts/font_loader.py to project root
...
def get_theme_font(size: int = 24, font_type: str = "primary") -> pygame.font.Font:
    ...
    theme = theme_loader.load_theme("menu", theme_name="pipboy")

    # Get font filename from theme
    font_key = f"{font_type}_font"
    font_filename = theme.get("style", {}).get("typography", {}).get(font_key)
...
def clear_render_cache():
    ...
    try:
        from nrhof.ui.fonts import _render_mixed_text_cached
