...
class WebflowClient:
    ...
    def __init__(self, config: WebflowConfig, logger: Optional[logging.Logger] = None):
        self.config = config
    ...
    def _find_or_create_band(self, band_name: str) -> Optional[str]:
    ...
    def _find_or_create_album(self, album_name: str, band_name: str) -> Optional[str]:
        ...
        try:
            ...
            if items:
                for item in items:
                    ...
                    if (field_data.get('name', '').lower() == album_name.lower() and
                        field_data.get('band', '').lower() == band_name.lower()):
    ...
    def get_collection_items(self, collection_id: str, limit: int = 100, offset: int = 0) -> Optional[List[Dict[str, Any]]]:
...
def create_webflow_client(config: Dict[str, Any], logger: Optional[logging.Logger] = None) -> Optional[WebflowClient]:
    ...
    if not all([api_token, site_id, collection_id_bands, collection_id_albums, collection_id_songs]):
        if logger:
