...
class WebflowCache:
    ...
    def __init__(self, cache_dir: str = "runtime/webflow_cache", logger: Optional[logging.Logger] = None):
    ...
    def get_collection(self, collection_name: str) -> Optional[List[Dict[str, Any]]]:
    ...
    def set_collection(self, collection_name: str, items: List[Dict[str, Any]], etag: Optional[str] = None):
    ...
    def get_etag(self, collection_name: str) -> Optional[str]:
...
class WebflowCacheManager:
    ...
    def __init__(self, webflow_client, cache: WebflowCache, logger: Optional[logging.Logger] = None):
        """Initialize cache manager.
    ...
    def refresh_all(self, force: bool = False) -> bool:
        ...
        try:
            ...
            bands = self._fetch_all_pages(
                self.client.config.collection_id_bands,
                'bands'
            )
            if bands:
            ...
            songs = self._fetch_all_pages(
                self.client.config.collection_id_songs,
                'songs'
            )
            if songs:
            ...
            albums = self._fetch_all_pages(
                self.client.config.collection_id_albums,
                'albums'
            )
            if albums:
    ...
    def get_bands(self, filter_list: Optional[str] = None) -> List[Dict[str, Any]]:
