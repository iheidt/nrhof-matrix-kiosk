...
class LifecycleManager:
    ...
    def register(self, phase: LifecyclePhase, name: str, callback: Callable[[HookContext], None],
                 priority: int = 0, once: bool = False) -> Hook:
        ...
        self._logger.debug(f"Registered hook '{name}' for {phase.name}", priority=priority, once=once)
        return hook
    ...
    def clear(self, phase: Optional[LifecyclePhase] = None):
...
def register_hook(phase: LifecyclePhase, name: str, callback: Callable[[HookContext], None],
                  priority: int = 0, once: bool = False) -> Hook:
